...
Explanations needed on why there is a need to connect R to C++ --> here (source : advanced R by Hadley ?)
...
----------------
  ...
  - Although it provides syntax comfort, sugar somewhat goes against one of the main message when 
    ...
    languages such as Julia (Bezanson, Karpinski, Shah and Edelman 20012) even go as far as advocating 
...
TODO: mention the unavoidable competition with the existing solution. 
...
TODO: do we create a specific blog => yes

TODO: articles in R journal (other ?). Blog post in the consortium blog. 

TODO: same strategy as `Rcpp` => keynotes in meetup ? 
...
on which `Rcpp` is based now are a legacy of my work in the project between 2011 and ... (?). To this day, my contributions to 
`Rcpp` still represent more commits than the next two developpers. I am very proud of the work done and its adoption
by the community. I am also frustrated by some choices that were made, and my main reason for stepping down (aside ?) from the 
