...
When these 3 versions are the same, everything works fine. When they start to differ, dealing with  complicated binary incompatibility problems start to happen. Package developpers
...
it makes sense to have a dedicated blog to document progress or highlight specific features being developped. We'll probably 
...
solution, originally inspired from the design of `rJava` (Urbanek 2016). I have been heavily involved 
in designing the fundamental principles of what `Rcpp` is now. Nowadays its maintainership is centered on backwards 
compatibility whereas there is room for (perhaps disruptive) innovation and redesign. 
